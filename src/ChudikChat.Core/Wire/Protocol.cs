...
public sealed record IdentifyFrame : WireFrame
{
    ...
    /// <summary>
    /// Операционная система звонящего: "windows", "macos" или "android".
    /// null — собеседник старой сборки или платформа, которой мы не знаем.
    /// </summary>
    /// <remarks>
    /// Строка, а не перечисление. Перечисление в этом контексте уехало бы числом —
    /// строкового конвертера у <see cref="WireJsonContext"/> нет, — а включи мы его,
    /// незнакомое значение из будущей сборки разворачивалось бы в исключение и рвало
    /// соединение целиком. Разбор в <see cref="Model.PeerPlatforms.Parse"/> вместо этого
    /// молча отвечает «неизвестно».
    /// </remarks>
